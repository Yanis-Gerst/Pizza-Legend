class Combatant {
      ...
      return [
        {
          ...
          specialDamage: true,
          onCaster: true,
          power: this.specialDamage.power,
        },
      ];
    }
  ...
  getBuff() {
    if (this.buff?.name === "Moment Chaud") {
      this.buff.buffAtk =
        1 + Math.round(this.buff.caster.attackSpe * this.buff.power) / 100;
    } else if (this.buff?.name === "AtkSpe +") {
      if (this.debuff.buffAtkSpe) return;
      this.buff.buffAtkSpe =
        1 + Math.round(this.buff.caster.attackSpe * this.buff.power) / 100;
    } else if (this.debuff?.name === "Atk -") {
      // Pourcentage
      this.debuff.debuffAtk =
        1 - Math.round(this.debuff.caster.attackSpe * this.debuff.power) / 100;
    } else if (this.debuff?.name === "AtkSpe -") {
      if (this.status.debuffAtkSpe) return;
      this.debuff.debuffAtkSpe =
        Math.round(this.debuff.caster.attackSpe * this.debuff.power) / 100;
    }
  }
  ...
  decrementStatus() {
    const expiredEvent = this.statusEffects.reduce(
      (previousEffect, currentEffect) => {
        if (typeof previousEffect === "object")
          return previousEffect.concat(this.decrementEffect(currentEffect));
        return this.decrementEffect(previousEffect).concat(
          this.decrementEffect(currentEffect)
        );
      }
    );
    return expiredEvent;
